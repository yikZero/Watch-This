...
5. **Format & send** — `buildNotificationText` in `index.ts` constructs a plain Telegram message with the header, date range, ordered TV list, and ordered movie list. Each ranked item is rendered as plain text `{name}  {rating}` — no inline links and no Infuse deep links. `sendTelegramNotification` posts via Telegram Bot API `sendMessage`; `TELEGRAM_MESSAGE_THREAD_ID` routes the message into a group topic.
