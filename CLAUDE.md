...
Bun runs `.ts` natively and auto-loads `.env`; there is no build step and no `dotenv` import. Requires `.env` with `OPENROUTER_API_KEY`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`, and optionally `TELEGRAM_MESSAGE_THREAD_ID` for group topics. See `.env.example`.
...
5. **Format & send** — `buildNotificationText` in `index.ts` constructs an HTML-formatted Telegram message with a bold header, date range, ordered TV list, and ordered movie list. Each ranked item is rendered as plain text `{name}  {rating}` — no inline links and no Infuse deep links. `sendTelegramNotification` posts via Telegram Bot API `sendMessage` with `parse_mode: "HTML"`; `TELEGRAM_MESSAGE_THREAD_ID` routes the message into a group topic.
